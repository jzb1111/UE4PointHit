...
class GetPoint:
    def begin_play(self):
        self.uobject.get_owner().bind_event('EventGetLocation',self.calc_location)
        #np.save('D:/location',np.zeros((2)))
    def calc_location(self,location):
        
        #location = self.uobject.location
        #print("hilocation")
        np.save('D:/location',location)
